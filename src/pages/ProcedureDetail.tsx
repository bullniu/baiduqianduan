                ...
                {layer.passes.length > 0 ? (
                  <>
                    ...
                    <div className="md:hidden space-y-3 p-4">
                      {layer.passes.map((pass) => (
                        <div key={pass.id} className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
                          {/* 道号和删除按钮 */}
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                          ...
                          <div className="flex items-center justify-between bg-slate-50 rounded-lg p-3">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm text-slate-600">计时</span>
                            </div>
                            <div className="flex items-center space-x-3">
                              <div className={`px-4 py-2 rounded font-mono font-bold text-xl min-w-[80px] text-center ${
                                isTimerRunning(pass.id)
                                  ? 'bg-green-100 text-green-700 animate-pulse'
                                  : 'bg-white text-slate-700 border border-slate-200'
                              }`}>
                                {getTimerDisplay(pass.id)}
                              </div>
                              <div className="flex items-center space-x-2">
                                {!isTimerRunning(pass.id) ? (
                                  <button
                                  ...
                                  <button
                                    onClick={() => pauseTimer(pass.id, layer.id)}
                                    className="p-2.5 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-colors"
                                    title="暂停计时"
                                  >
                                ...
                                <button
                                  onClick={() => resetTimer(pass.id, layer.id)}
                                  className="p-2.5 bg-slate-400 text-white rounded-lg hover:bg-slate-500 transition-colors"
                                  title="重置计时器"
                                >
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            <div>
                              <label className="block text-xs text-slate-500 mb-1">电流 (A)</label>
                              <input
                                type="number"
                                value={pass.current || ''}
                                onChange={(e) => handlePassChange(layer.id, pass.id, 'current', e.target.value)}
                                className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-right text-lg"
                                placeholder="0"
                                step="0.1"
                              />
                            </div>
                            <div>
                              <label className="block text-xs text-slate-500 mb-1">电压 (V)</label>
                              <input
                                type="number"
                                value={pass.voltage || ''}
                                onChange={(e) => handlePassChange(layer.id, pass.id, 'voltage', e.target.value)}
                            ...
                            <div>
                              <label className="block text-xs text-slate-500 mb-1">长度 (cm)</label>
                              <input
                                type="number"
                                value={pass.weldLength || ''}
                                onChange={(e) => handlePassChange(layer.id, pass.id, 'weldLength', e.target.value)}
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="bg-teal-50 rounded-lg p-3">
                              <div className="text-xs text-teal-600 mb-1">焊接速度 (cm/min)</div>
                              <div className="font-mono font-bold text-xl text-teal-700 text-right">
                                {getLiveTravelSpeed(pass, pass.id).toFixed(2)}
                              </div>
                            </div>
                            <div className="bg-blue-50 rounded-lg p-3">
                              <div className="text-xs text-blue-600 mb-1">热输入 (kJ/cm)</div>
                              <div className="font-mono font-bold text-xl text-blue-700 text-right">
                                {getLiveHeatInput(pass, pass.id).toFixed(3)}
                              </div>
                            </div>
                          </div>
